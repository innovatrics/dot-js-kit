const Pr = /* @__PURE__ */ Symbol("Comlink.proxy"), wo = /* @__PURE__ */ Symbol("Comlink.endpoint"), Po = /* @__PURE__ */ Symbol("Comlink.releaseProxy"), xe = /* @__PURE__ */ Symbol("Comlink.finalizer"), Ne = /* @__PURE__ */ Symbol("Comlink.thrown"), jr = (y) => typeof y == "object" && y !== null || typeof y == "function", jo = {
  canHandle: (y) => jr(y) && y[Pr],
  serialize(y) {
...
function pt(y, a = globalThis, r = ["*"]) {
  a.addEventListener("message", function t(e) {
    ...
    let h;
    try {
      const S = u.slice(0, -1).reduce((w, j) => w[j], y), g = u.reduce((w, j) => w[j], y);
      switch (o) {
        case "GET":
          h = g;
          break;
...
function ce(y) {
  switch (y.type) {
    case "HANDLER":
      return Ir.get(y.name).deserialize(y.value);
    case "RAW":
      return y.value;
  }
}
...
var ri = (() => {
  ...
  return (async function(a = {}) {
    ...
    var j, I;
    (u || s) && (s ? g = self.location.href : typeof document < "u" && document.currentScript && (g = document.currentScript.src), y && (g = y), g.startsWith("blob:") ? g = "" : g = g.slice(0, g.replace(/[?#].*/, "").lastIndexOf("/") + 1), s && (I = (c) => {
    ...
    var Ot = (c) => {
      for (; c.length > 0; )
        c.shift()(t);
    }, wt = [], Ur = (c) => wt.unshift(c), Pt = [], Gr = (c) => Pt.unshift(c), jt = t.noExitRuntime || !0;
    class $r {
      constructor(l) {
    ...
    function Jr(c, l, p = {}) {
      ...
      if (le[c] = l, delete Ie[c], pe.hasOwnProperty(c)) {
        var b = pe[c];
...
` + b.replace(/^Error(:[^\n]*)?\n/, ""));
    ...
    }, Ut = (c, l, p) => (c instanceof Object || x(`${p} with invalid "this": ${c}`), c instanceof l.registeredClass.constructor || x(`${p} incompatible with "this" of type ${c.constructor.name}`), c.$$.ptr || x(`cannot call emscripten binding method ${p} on deleted object`), De(c.$$.ptr, c.$$.ptrType.registeredClass, l.registeredClass)), In = (c, l, p, m, b, P, C, v, k, R) => {
    ...
    var mo = { g: Br, s: Hr, n: Zr, w: Kr, f: bn, d: jn, a: In, u: Tn, l: An, h: En, m: Ln, b: _n, e: Fn, c: xn, v: Gn, i: Kn, x: qn, q: Xn, j: eo, y: Je, k: to, o: oo, A: io, z: ao, r: lo, t: fo, p: Wt }, F = await zr();
    F.C;
