...
var xo = (p, c, n) => c in p ? Ro(p, c, { enumerable: !0, configurable: !0, writable: !0, value: n }) : p[c] = n;
var ze = (p, c, n) => xo(p, typeof c != "symbol" ? c + "" : c, n), ir = (p, c, n) => c.has(p) || or("Cannot " + n);
var ee = (p, c, n) => (ir(p, c, "read from private field"), n ? n.call(p) : c.get(p)), Ve = (p, c, n) => c.has(p) ? or("Cannot add the same private member more than once") : c instanceof WeakSet ? c.add(p) : c.set(p, n), Ue = (p, c, n, t) => (ir(p, c, "write to private field"), t ? t.call(p, n) : c.set(p, n), n);
/**
 * @license
 * Copyright 2019 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
const _r = Symbol("Comlink.proxy"), No = Symbol("Comlink.endpoint"), Wo = Symbol("Comlink.releaseProxy"), He = Symbol("Comlink.finalizer"), Ye = Symbol("Comlink.thrown"), Lr = (p) => typeof p == "object" && p !== null || typeof p == "function", zo = {
...
function Pt(p, c = globalThis, n = ["*"]) {
  c.addEventListener("message", function t(e) {
    ...
    Promise.resolve(g).catch((S) => ({ value: S, [Ye]: 0 })).then((S) => {
      const [h, O] = Ze(S);
      c.postMessage(Object.assign(Object.assign({}, h), { id: r }), O), o === "RELEASE" && (c.removeEventListener("message", t), Rr(c), He in p && typeof p[He] == "function" && p[He]());
    }).catch((S) => {
      const [h, O] = Ze({
...
const Je = /* @__PURE__ */ new WeakMap(), Ke = "FinalizationRegistry" in globalThis && new FinalizationRegistry((p) => {
  const c = (Je.get(p) || 0) - 1;
  Je.set(p, c), c === 0 && xr(p);
});
function Bo(p, c) {
  const n = (Je.get(c) || 0) + 1;
  Je.set(c, n), Ke && Ke.register(p, c, p);
}
function Ho(p) {
  Ke && Ke.unregister(p);
}
function wt(p, c, n = [], t = function() {
}) {
  let e = !1;
  const r = new Proxy(t, {
    get(o, u) {
...
function ar(p) {
  const c = p.map(Ze);
  return [c.map((n) => n[0]), Yo(c.map((n) => n[1]))];
}
...
class ei {
  ...
  writeImageToMemory(c) {
    ...
    const n = new Qo(this.samWasmModule, c);
...
const Be = (p, c) => Math.hypot(c.x - p.x, c.y - p.y), ti = (p) => {
  const { bottomLeft: c, bottomRight: n, topLeft: t, topRight: e } = p, r = Be(t, e), o = Be(e, n), u = Be(c, n), s = Be(t, c);
  ...
  function p(r) {
    return typeof Float32Array < "u" ? function() {
      ...
      function u(s, g, S, h, O) {
        var I = s(h, O + g), j = s(h, O + S), F = (j >> 31) * 2 + 1, P = j >>> 20 & 2047, k = 4294967296 * (j & 1048575) + I;
        return P === 2047 ? k ? NaN : F * (1 / 0) : P === 0 ? F * 5e-324 * k : F * Math.pow(2, P - 1075) * (k + 4503599627370496);
      }
...
function pe() {
  return Or || (Or = 1, function(p) {
    var c = p;
    c.asPromise = ri(), c.base64 = ni(), c.EventEmitter = oi(), c.float = ii(), c.inquire = ai(), c.utf8 = si(), c.pool = li(), c.LongBits = ci(), c.isNode = !!(typeof be < "u" && be && be.process && be.process.versions && be.process.versions.node), c.global = c.isNode && be || typeof window < "u" && window || typeof self < "u" && self || ct, c.emptyArray = Object.freeze ? Object.freeze([]) : (
...
function Wr() {
  ...
  function r(P, k, R) {
    this.fn = P, this.len = k, this.next = void 0, this.val = R;
  }
  function o() {
  }
  function u(P) {
  ...
  }, p.Array !== Array && (s.alloc = p.pool(s.alloc, p.Array.prototype.subarray)), s.prototype._push = function(P, k, R) {
    return this.tail = this.tail.next = new r(P, k, R), this.len += k, this;
  };
  function S(P, k, R) {
    k[R] = P & 255;
  }
  ...
  function I(P, k, R) {
    for (; P.hi; )
      k[R++] = P.lo & 127 | 128, P.lo = (P.lo >>> 7 | P.hi << 25) >>> 0, P.hi >>>= 7;
    for (; P.lo > 127; )
      k[R++] = P.lo & 127 | 128, P.lo = P.lo >>> 7;
    k[R++] = P.lo;
  }
  s.prototype.uint64 = function(P) {
    ...
    var k = n.from(P).zzEncode();
    return this._push(I, k.length(), k);
  }, s.prototype.bool = function(P) {
    return this._push(S, 1, P ? 1 : 0);
  };
  function j(P, k, R) {
    k[R] = P & 255, k[R + 1] = P >>> 8 & 255, k[R + 2] = P >>> 16 & 255, k[R + 3] = P >>> 24;
  }
  ...
  var F = p.Array.prototype.set ? function(P, k, R) {
    k.set(P, R);
  } : function(P, k, R) {
  ...
  return s.prototype.bytes = function(P) {
    ...
    if (p.isString(P)) {
      var R = s.alloc(k = t.length(P));
      t.decode(P, R, 0), P = R;
    }
    return this.uint32(k)._push(F, k, P);
  }, s.prototype.string = function(P) {
    var k = e.length(P);
    return k ? this.uint32(k)._push(e.write, k, P) : this._push(S, 1, 0);
    ...
    var P = this.head, k = this.tail, R = this.len;
    return this.reset().uint32(R), R && (this.tail.next = P.next, this.tail = k, this.len += R), this;
  }, s.prototype.finish = function() {
    for (var P = this.head.next, k = this.constructor.alloc(this.len), R = 0; P; )
      P.fn(P.val, k, R), R += P.len, P = P.next;
    return k;
...
var kr, Ar;
function mi() {
  return Ar || (Ar = 1, kr = {}), kr;
}
var Tr;
function yi() {
  return Tr || (Tr = 1, function(p) {
    var c = p;
    c.build = "minimal", c.Writer = Wr(), c.BufferWriter = ui(), c.Reader = zr(), c.BufferReader = di(), c.util = pe(), c.rpc = pi(), c.roots = mi(), c.configure = n;
    function n() {
...
var gi = (() => {
  ...
  return async function(c = {}) {
    ...
    var k = t.wasmBinary, R, B = !1, De, K, $, me, we, re, N, It, jt, Ct, St, Dt = (l) => l.startsWith("file://");
    function kt() {
      var l = R.buffer;
      t.HEAP8 = K = new Int8Array(l), t.HEAP16 = me = new Int16Array(l), t.HEAPU8 = $ = new Uint8Array(l), t.HEAPU16 = we = new Uint16Array(l), t.HEAP32 = re = new Int32Array(l), t.HEAPU32 = N = new Uint32Array(l), t.HEAPF32 = It = new Float32Array(l), t.HEAPF64 = St = new Float64Array(l), t.HEAP64 = jt = new BigInt64Array(l), t.HEAPU64 = Ct = new BigUint64Array(l);
    }
    function Vr() {
      if (t.preRun)
    ...
    async function Kr(l, a) {
      try {
        var f = await Jr(l), m = await WebAssembly.instantiate(f, a);
        return m;
      } catch (b) {
    ...
    async function Zr(l, a, f) {
      if (!l && typeof WebAssembly.instantiateStreaming == "function" && !Dt(a))
        try {
    ...
    async function Xr() {
      function l(w, C) {
        return M = w.exports, M = A.instrumentWasmExports(M), R = M.D, kt(), M.J, Br(), M;
      }
      Gr();
      function a(w) {
        return l(w.instance);
    ...
    var Tt = (l) => {
      for (; l.length > 0; )
        l.shift()(t);
    }, Et = [], Qr = (l) => Et.unshift(l), Ft = [], en = (l) => Ft.unshift(l), _t = t.noExitRuntime || !0;
    class tn {
      constructor(a) {
      ...
      set_caught(a) {
        a = a ? 1 : 0, K[this.ptr + 12] = a;
      }
      get_caught() {
        return K[this.ptr + 12] != 0;
      }
      set_rethrown(a) {
        a = a ? 1 : 0, K[this.ptr + 13] = a;
      }
      get_rethrown() {
    ...
    var Lt = 0, rn = (l, a, f) => {
      ...
      function m(v) {
        var D = f(v);
        D.length !== l.length && Ee("Mismatched type converter count");
        for (var _ = 0; _ < l.length; ++_)
          Y(l[_], D[_]);
      }
      var b = new Array(a.length), w = [], C = 0;
      a.forEach((v, D) => {
        ue.hasOwnProperty(v) ? b[D] = ue[v] : (w.push(v), ye.hasOwnProperty(v) || (ye[v] = []), ye[v].push(() => {
          b[D] = ue[v], ++C, C === w.length && m(b);
        }));
      }), w.length === 0 && m(b);
    };
    function an(l, a, f = {}) {
    ...
    var xt = (l, a, f) => {
      ...
      }, argPackAdvance: Z, readValueFromPointer: xt(a, f, !w), destructorFunction: null });
    }, Z = 8, ln = (l, a, f, m) => {
      a = z(a), Y(l, { name: a, fromWireType: function(b) {
        return !!b;
      }, toWireType: function(b, w) {
        return w ? f : m;
      }, argPackAdvance: Z, readValueFromPointer: function(b) {
        return this.fromWireType($[b]);
      }, destructorFunction: null });
    }, cn = (l) => ({ count: l.count, deleteScheduled: l.deleteScheduled, preservePointerOnDelete: l.preservePointerOnDelete, ptr: l.ptr, ptrType: l.ptrType, smartPtr: l.smartPtr, smartPtrType: l.smartPtrType }), Xe = (l) => {
      function a(f) {
    ...
    function mn(l) {
      ...
      var f = pn(this.registeredClass, a);
      if (f !== void 0) {
        if (f.$$.count.value === 0)
      ...
      var w = this.registeredClass.getActualType(a), C = Vt[w];
      if (!C)
        return b.call(this);
      var v;
      this.isConst ? v = C.constPointerType : v = C.pointerType;
      var D = zt(a, this.registeredClass, v.registeredClass);
      return D === null ? b.call(this) : this.isSmartPointer ? Fe(v.registeredClass.instancePrototype, { ptrType: v, ptr: D, smartPtrType: this, smartPtr: l }) : Fe(v.registeredClass.instancePrototype, { ptrType: v, ptr: D });
    }
    var Ie = (l) => typeof FinalizationRegistry > "u" ? (Ie = (a) => a, l) : (Qe = new FinalizationRegistry((a) => {
      ...
      Object.assign(_e.prototype, { isAliasOf(l) {
        ...
        var l = Ie(Object.create(Object.getPrototypeOf(this), { $$: { value: cn(this.$$) } }));
    ...
    var Le = (l, a) => Object.defineProperty(a, "name", { value: l }), hn = (l, a, f) => {
      ...
      t.hasOwnProperty(l) ? ((f === void 0 || t[l].overloadTable !== void 0 && t[l].overloadTable[f] !== void 0) && x(`Cannot register public name '${l}' twice`), hn(t, l, l), t[l].overloadTable.hasOwnProperty(f) && x(`Cannot register multiple overloads of a function with the same number of arguments (${f})!`), t[l].overloadTable[f] = a) : (t[l] = a, t[l].argCount = f);
    }, gn = 48, bn = 57, vn = (l) => {
      l = l.replace(/[^a-zA-Z0-9_]/g, "$");
      var a = l.charCodeAt(0);
      return a >= gn && a <= bn ? `_${l}` : l;
    };
    function On(l, a, f, m, b, w, C, v) {
    ...
    var jn = () => {
      ...
      }, argPackAdvance: Z, readValueFromPointer: Re, fromWireType: mn });
    };
    function xe(l, a, f, m, b, w, C, v, D, _, T) {
      this.name = l, this.registeredClass = a, this.isReference = f, this.isConst = m, this.isSmartPointer = b, this.pointeeType = w, this.sharingPolicy = C, this.rawGetPointee = v, this.rawConstructor = D, this.rawShare = _, this.rawDestructor = T, !b && a.baseClass === void 0 ? m ? (this.toWireType = wn, this.destructorFunction = null) : (this.toWireType = In, this.destructorFunction = null) : this.toWireType = Pn;
    }
    var Ut = (l, a, f) => {
...
` + b.replace(/^Error(:[^\n]*)?\n/, ""));
      ...
      throw a.forEach(b), new $t(`${l}: ` + f.map(Gt).join([", "]));
    }, An = (l, a, f, m, b, w, C, v, D, _, T, W, V) => {
      T = z(T), w = ne(b, w), v && (v = ne(C, v)), _ && (_ = ne(D, _)), V = ne(W, V);
      var q = vn(T);
      et(q, function() {
        ...
        var Q = Le(T, function(...lt) {
          ...
          if (nr === void 0)
            throw new he(`Tried to invoke ctor of ${T} with invalid number of parameters (${lt.length}) - expected (${Object.keys(U.constructor_body).toString()}) parameters instead!`);
          return nr.apply(this, lt);
        }), ae = Object.create(H, { constructor: { value: Q } });
        Q.prototype = ae;
        var U = new On(T, Q, ae, V, ie, w, v, _);
        U.baseClass && ((rr = U.baseClass).__derivedClasses ?? (rr.__derivedClasses = []), U.baseClass.__derivedClasses.push(U));
        var se = new xe(T, U, !0, !1, !1), We = new xe(T + "*", U, !1, !1, !1), tr = new xe(T + " const*", U, !1, !0, !1);
        return Vt[l] = { pointerType: We, constPointerType: tr }, Ut(q, Q), [se, We, tr];
      });
    }, Bt = (l, a) => {
      for (var f = [], m = 0; m < l; m++)
        f.push(N[a + m * 4 >> 2]);
    ...
    var Ne = (l) => {
      ...
      if (l instanceof At || l == "unwind")
        return De;
      h(1, l);
    }, Yt = 0, Jt = () => _t || Yt > 0, Kt = (l) => {
      var a;
      De = l, Jt() || ((a = t.onExit) == null || a.call(t, l), B = !0), h(l, new At(l));
    }, En = (l, a) => {
      De = l, Kt(l);
    }, Fn = En, _n = () => {
      if (!Jt())
      ...
      var a = A.callStackNameToId[l];
      return a === void 0 && (a = A.callStackId++, A.callStackNameToId[l] = a, A.callStackIdToName[a] = l), a;
    }, maybeStopUnwind() {
      A.currData && A.state === A.State.Unwinding && A.exportCallStack.length === 0 && (A.state = A.State.Normal, Ne(_o), typeof Fibers < "u" && Fibers.trampoline());
    }, whenDone() {
      return new Promise((l, a) => {
        A.asyncPromiseHandlers = { resolve: l, reject: a };
      });
    }, allocateData() {
      var l = at(12 + A.StackSize);
      return A.setDataHeader(l, l + 12, A.StackSize), A.setDataRewindFunc(l), l;
    }, setDataHeader(l, a, f) {
      N[l >> 2] = a, N[l + 4 >> 2] = a + f;
    }, setDataRewindFunc(l) {
      var a = A.exportCallStack[0], f = A.getCallStackId(a);
      re[l + 8 >> 2] = f;
    }, getDataRewindFuncName(l) {
      var a = re[l + 8 >> 2], f = A.callStackIdToName[a];
      return f;
    }, getDataRewindFunc(l) {
      var a = M[l];
      return a;
    }, doRewind(l) {
      var a = A.getDataRewindFuncName(l), f = A.getDataRewindFunc(a);
      return f();
    }, handleSleep(l) {
      if (!B) {
        if (A.state === A.State.Normal) {
          var a = !1, f = !1;
          l((m = 0) => {
            if (!B && (A.handleSleepReturnValue = m, a = !0, !!f)) {
              A.state = A.State.Rewinding, Ne(() => Lo(A.currData)), typeof MainLoop < "u" && MainLoop.func && MainLoop.resume();
              var b, w = !1;
              try {
              ...
              if (!A.currData) {
                ...
                v && (A.asyncPromiseHandlers = null, (w ? v.reject : v.resolve)(b), C = !0);
              }
              if (w && !C)
                throw b;
            }
          }), f = !0, a || (A.state = A.State.Unwinding, A.currData = A.allocateData(), typeof MainLoop < "u" && MainLoop.func && MainLoop.pause(), Ne(() => Fo(A.currData)));
        } else A.state === A.State.Rewinding ? (A.state = A.State.Normal, Ne(Mo), te(A.currData), A.currData = null, A.sleepCallbacks.forEach(Zt)) : ke(`invalid state: ${A.state}`);
        return A.handleSleepReturnValue;
      }
    }, handleAsync(l) {
      return A.handleSleep((a) => {
        l().then(a);
      });
    } };
    function qt(l, a, f, m, b, w) {
      var C = a.length;
      C < 2 && x("argTypes array size mismatch! Must at least get return value and 'this' types!"), a[1];
      var v = Tn(a), D = a[0].name !== "void", _ = C - 2, T = new Array(_), W = [], V = [], q = function(...X) {
        V.length = 0;
        var ie;
        W.length = 1, W[0] = b;
        for (var H = 0; H < _; ++H)
    ...
    var Ln = (l, a, f, m, b, w) => {
      ...
      b = ne(m, b), ge([], [l], (v) => {
        ...
        }, ge([], C, (_) => (_.splice(1, 0, null), v.registeredClass.constructor_body[a - 1] = qt(D, _, null, b, w), [])), [];
      });
    }, Xt = (l, a, f) => (l instanceof Object || x(`${f} with invalid "this": ${l}`), l instanceof a.registeredClass.constructor || x(`${f} incompatible with "this" of type ${l.constructor.name}`), l.$$.ptr || x(`cannot call emscripten binding method ${f} on deleted object`), Me(l.$$.ptr, l.$$.ptrType.registeredClass, a.registeredClass)), Mn = (l, a, f, m, b, w, C, v, D, _) => {
      a = z(a), b = ne(m, b), ge([], [l], (T) => {
        T = T[0];
        var W = `${T.name}.${a}`, V = { get() {
          je(`Cannot access ${W} due to unbound types`, [f, C]);
        }, enumerable: !0, configurable: !0 };
        return D ? V.set = () => je(`Cannot access ${W} due to unbound types`, [f, C]) : V.set = (q) => x(W + " is a read-only property"), Object.defineProperty(T.registeredClass.instancePrototype, a, V), ge([], D ? [f, C] : [f], (q) => {
          var X = q[0], ie = { get() {
            var Q = Xt(this, T, W + " getter");
          ...
          if (D) {
            ...
            var H = q[1];
            ie.set = function(Q) {
              var ae = Xt(this, T, W + " setter"), U = [];
              D(_, ae, H.toWireType(U, Q)), tt(U);
            };
          }
      ...
      l > 9 && --oe[l + 1] === 0 && (oe[l] = void 0, rt.push(l));
    }, Rn = () => oe.length / 2 - 5 - rt.length, xn = () => {
      oe.push(0, 1, void 0, 1, null, 1, !0, 1, !1, 1), t.count_emval_handles = Rn;
      ...
      switch (l) {
        ...
        case !1:
          return 8;
        default: {
          const a = rt.pop() || oe.length;
          return oe[a] = l, oe[a + 1] = 1, a;
        }
      }
    } }, Nn = { name: "emscripten::val", fromWireType: (l) => {
      var a = J.toValue(l);
      return nt(l), a;
    }, toWireType: (l, a) => J.toHandle(a), argPackAdvance: Z, readValueFromPointer: Re, destructorFunction: null }, Wn = (l) => Y(l, Nn), zn = (l, a, f) => {
      switch (a) {
        case 1:
          return f ? function(m) {
      ...
      }, toWireType: (w, C) => C.value, argPackAdvance: Z, readValueFromPointer: zn(a, f, m), destructorFunction: null }), et(a, b);
    }, ot = (l, a) => {
      var f = ue[l];
      ...
      switch (a) {
        case 4:
          return function(f) {
            return this.fromWireType(It[f >> 2]);
          };
        case 8:
          return function(f) {
            return this.fromWireType(St[f >> 3]);
          };
        default:
          throw new TypeError(`invalid float width (${a}): ${l}`);
      }
    }, Gn = (l, a, f) => {
      a = z(a), Y(l, { name: a, fromWireType: (m) => m, toWireType: (m, b) => b, argPackAdvance: Z, readValueFromPointer: $n(a, f), destructorFunction: null });
    }, Bn = (l) => {
      l = l.trim();
      const a = l.indexOf("(");
      ...
      var v = a.includes("unsigned"), D = (T, W) => {
      }, _;
      v ? _ = function(T, W) {
        return D(W, this.name), W >>> 0;
      } : _ = function(T, W) {
        return D(W, this.name), W;
      }, Y(l, { name: a, fromWireType: w, toWireType: _, argPackAdvance: Z, readValueFromPointer: xt(a, f, m !== 0), destructorFunction: null });
    }, Jn = (l, a, f) => {
      ...
      function w(C) {
        var v = N[C >> 2], D = N[C + 4 >> 2];
        return new b(K.buffer, D, v);
      }
      f = z(f), Y(l, { name: f, fromWireType: w, argPackAdvance: Z, readValueFromPointer: w }, { ignoreDuplicateRegistrations: !0 });
    }, Kn = (l, a, f, m) => {
      if (!(m > 0)) return 0;
      ...
      return a[f] = 0, f - b;
    }, Zn = (l, a, f) => Kn(l, $, a, f), qn = (l) => {
      for (var a = 0, f = 0; f < l.length; ++f) {
        var m = l.charCodeAt(f);
        m <= 127 ? a++ : m <= 2047 ? a += 2 : m >= 55296 && m <= 57343 ? (a += 4, ++f) : a += 3;
      }
      return a;
    }, Qt = typeof TextDecoder < "u" ? new TextDecoder() : void 0, Xn = (l, a = 0, f = NaN) => {
      for (var m = a + f, b = a; l[b] && !(b >= m); ) ++b;
      if (b - a > 16 && l.buffer && Qt)
        return Qt.decode(l.subarray(a, b));
      for (var w = ""; a < b; ) {
        var C = l[a++];
        if (!(C & 128)) {
        ...
        var D = l[a++] & 63;
        if ((C & 240) == 224 ? C = (C & 15) << 12 | v << 6 | D : C = (C & 7) << 18 | v << 12 | D << 6 | l[a++] & 63, C < 65536)
          w += String.fromCharCode(C);
        else {
          var _ = C - 65536;
      ...
      a = z(a), Y(l, { name: a, fromWireType(f) {
        ...
        var C = at(4 + b + 1), v = C + 4;
        if (N[C >> 2] = b, w)
          Zn(m, v, b + 1);
        else if (w)
          for (var D = 0; D < b; ++D) {
            var _ = m.charCodeAt(D);
            _ > 255 && (te(C), x("String has UTF-16 code units that do not fit in 8 bits")), $[v + D] = _;
          }
        else
          for (var D = 0; D < b; ++D)
            $[v + D] = m[D];
        return f !== null && f.push(te, C), C;
      }, argPackAdvance: Z, readValueFromPointer: Re, destructorFunction(f) {
        te(f);
      } });
    }, er = typeof TextDecoder < "u" ? new TextDecoder("utf-16le") : void 0, to = (l, a) => {
      for (var f = l, m = f >> 1, b = m + a / 2; !(m >= b) && we[m]; ) ++m;
      if (f = m << 1, f - l > 32 && er) return er.decode($.subarray(l, f));
      ...
      f = z(f);
      var m, b, w, C;
      a === 2 ? (m = to, b = ro, C = no, w = (v) => we[v >> 1]) : a === 4 && (m = oo, b = io, C = ao, w = (v) => N[v >> 2]), Y(l, { name: f, fromWireType: (v) => {
        for (var D = N[v >> 2], _, T = v + 4, W = 0; W <= D; ++W) {
          var V = v + 4 + W * a;
          if (W == D || w(V) == 0) {
            var q = V - T, X = m(T, q);
        ...
        var _ = C(D), T = at(4 + _ + a);
        return N[T >> 2] = _ / a, b(D, T + 4, _ + a), v !== null && v.push(te, T), T;
      }, argPackAdvance: Z, readValueFromPointer: Re, destructorFunction(v) {
        te(v);
      } });
    }, lo = (l, a) => {
    ...
    }, fo = (l, a, f) => (l = J.toValue(l), a = ot(a, "emval::as"), uo(a, f, l)), po = (l, a) => (l = J.toValue(l), a = J.toValue(a), J.toHandle(l[a])), mo = {}, yo = (l) => {
      var a = mo[l];
      return a === void 0 ? z(l) : a;
    }, ho = (l) => J.toHandle(yo(l)), go = (l) => {
      var a = J.toValue(l);
      tt(a), nt(l);
    }, bo = (l, a) => {
      l = ot(l, "_emval_take_value");
      var f = l.readValueFromPointer(a);
      return J.toHandle(f);
    }, Ce = {}, vo = () => performance.now(), Oo = (l, a) => {
      if (Ce[l] && (clearTimeout(Ce[l].id), delete Ce[l]), !a) return 0;
      var f = setTimeout(() => {
        delete Ce[l], Zt(() => Eo(l, vo()));
      }, a);
      return Ce[l] = { id: f, timeout_ms: a }, 0;
    }, wo = () => 2147483648, Po = (l, a) => Math.ceil(l / a) * a, Io = (l) => {
      var a = R.buffer, f = (l - a.byteLength + 65535) / 65536 | 0;
      try {
        return R.grow(f), kt(), 1;
      } catch {
      }
    }, jo = (l) => {
      var a = $.length;
      ...
      for (var m = 1; m <= 4; m *= 2) {
        ...
        var w = Math.min(f, Po(Math.max(l, b), 65536)), C = Io(w);
        if (C)
          return !0;
      }
      return !1;
    }, it = {}, Co = () => S || "./this.program", Se = () => {
      if (!Se.strings) {
        var l = (typeof navigator == "object" && navigator.languages && navigator.languages[0] || "C").replace("-", "_") + ".UTF-8", a = { USER: "web_user", LOGNAME: "web_user", PATH: "/", PWD: "/", HOME: "/home/web_user", LANG: l, _: Co() };
        for (var f in it)
          it[f] === void 0 ? delete a[f] : a[f] = it[f];
        var m = [];
        for (var f in a)
          m.push(`${f}=${a[f]}`);
        Se.strings = m;
      ...
      return Se().forEach((m, b) => {
        var w = a + f;
        N[l + b * 4 >> 2] = w, So(m, w), f += m.length + 1;
      }), 0;
    }, ko = (l, a) => {
      var f = Se();
      N[l >> 2] = f.length;
    ...
    on(), he = t.BindingError = class extends Error {
      constructor(a) {
    ...
    }, yn(), jn(), $t = t.UnboundTypeError = kn(Error, "UnboundTypeError"), xn();
    var Ao = { n: rn, w: nn, m: sn, z: ln, f: An, e: Ln, a: Mn, x: Wn, k: Vn, g: Un, l: Gn, b: Hn, d: Yn, c: Jn, y: eo, h: so, A: lo, r: co, i: fo, B: nt, j: po, p: ho, o: go, C: bo, s: Oo, v: jo, t: Do, u: ko, q: Kt }, M = await Xr();
    M.E;
    var To = M.F, at = t._malloc = M.G, te = t._free = M.H, Eo = M.I;
    ...
    var Fo = M.ea, _o = M.fa, Lo = M.ga, Mo = M.ha;
    function st() {
      if (ce > 0) {
        Pe = st;
        return;
      ...
      function l() {
        var a;
        t.calledRun = !0, !B && (Ur(), e(t), (a = t.onRuntimeInitialized) == null || a.call(t), $r());
      }
