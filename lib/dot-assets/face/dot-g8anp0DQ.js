...
var Lo = (f, s, r) => s in f ? Mo(f, s, { enumerable: !0, configurable: !0, writable: !0, value: r }) : f[s] = r;
var We = (f, s, r) => Lo(f, typeof s != "symbol" ? s + "" : s, r), nr = (f, s, r) => s.has(f) || rr("Cannot " + r);
var X = (f, s, r) => (nr(f, s, "read from private field"), r ? r.call(f) : s.get(f)), ze = (f, s, r) => s.has(f) ? rr("Cannot add the same private member more than once") : s instanceof WeakSet ? s.add(f) : s.set(f, r), Ue = (f, s, r, t) => (nr(f, s, "write to private field"), t ? t.call(f, r) : s.set(f, r), r);
/**
 * @license
 * Copyright 2019 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
const Er = Symbol("Comlink.proxy"), _o = Symbol("Comlink.endpoint"), Ro = Symbol("Comlink.releaseProxy"), Ge = Symbol("Comlink.finalizer"), $e = Symbol("Comlink.thrown"), Mr = (f) => typeof f == "object" && f !== null || typeof f == "function", Fo = {
  canHandle: (f) => Mr(f) && f[Er],
  serialize(f) {
...
function vt(f, s = globalThis, r = ["*"]) {
  s.addEventListener("message", function t(e) {
    ...
    let h;
    try {
      const S = u.slice(0, -1).reduce((O, I) => O[I], f), y = u.reduce((O, I) => O[I], f);
      switch (o) {
        case "GET":
          h = y;
          break;
    ...
    Promise.resolve(h).catch((S) => ({ value: S, [$e]: 0 })).then((S) => {
      const [y, O] = Ye(S);
      s.postMessage(Object.assign(Object.assign({}, y), { id: n }), O), o === "RELEASE" && (s.removeEventListener("message", t), _r(s), Ge in f && typeof f[Ge] == "function" && f[Ge]());
    }).catch((S) => {
      const [y, O] = Ye({
        value: new TypeError("Unserializable return value"),
        [$e]: 0
      });
      s.postMessage(Object.assign(Object.assign({}, y), { id: n }), O);
    });
  }), s.start && s.start();
}
function Wo(f) {
...
const Be = /* @__PURE__ */ new WeakMap(), He = "FinalizationRegistry" in globalThis && new FinalizationRegistry((f) => {
  const s = (Be.get(f) || 0) - 1;
  Be.set(f, s), s === 0 && Rr(f);
});
function Uo(f, s) {
  const r = (Be.get(s) || 0) + 1;
  Be.set(s, r), He && He.register(f, s, f);
}
function Vo(f) {
  He && He.unregister(f);
}
...
function or(f) {
  const s = f.map(Ye);
  return [s.map((r) => r[0]), Go(s.map((r) => r[1]))];
}
...
function de(f) {
  switch (f.type) {
    case "HANDLER":
      return Lr.get(f.name).deserialize(f.value);
    case "RAW":
      return f.value;
  }
}
...
class Zo {
  constructor(s, r) {
    ...
    ze(this, pe);
    ze(this, Oe);
    Ue(this, le, s), Ue(this, pe, this.allocate(r.length * r.BYTES_PER_ELEMENT)), Ue(this, Oe, this.allocate(r.length * r.BYTES_PER_ELEMENT));
  }
...
class Ko {
  ...
  async getOptimalRegionForCompressionDetectionFromDetectionCorners(s, r, t) {
    ...
    const e = this.convertToSamColorImage(s, r), { bottomLeft: n, topLeft: o, topRight: u } = t, a = [
      ...
      n.y - o.y
      // height
    ], { height: h, width: S, x: y, y: O } = await this.samWasmModule.selectDetailRegion(
      r.width,
      r.height,
      e.bgr0ImagePointer,
      a
...
const Ot = (f) => Number.parseFloat(f.toFixed(3)), xr = (f, s) => Math.min(f, s);
function qo(f, s) {
  const r = xr(s.width, s.height);
  return Ot(f * r);
}
const Xo = (f) => JSON.parse(
  JSON.stringify(f, (s, r) => typeof r == "number" ? Ot(r) : r)
);
...
function ei() {
  return cr || (cr = 1, function(f) {
    ...
    s.encode = function(o, u, a) {
      for (var h = null, S = [], y = 0, O = 0, I; u < a; ) {
        var j = o[u++];
        switch (O) {
          case 0:
            S[y++] = r[j >> 2], I = (j & 3) << 4, O = 1;
            break;
          case 1:
            S[y++] = r[I | j >> 4], I = (j & 15) << 2, O = 2;
            break;
          case 2:
            S[y++] = r[I | j >> 6], S[y++] = r[j & 63], O = 0;
            break;
        }
        y > 8191 && ((h || (h = [])).push(String.fromCharCode.apply(String, S)), y = 0);
      }
      return O && (S[y++] = r[I], S[y++] = 61, O === 1 && (S[y++] = 61)), h ? (y && h.push(String.fromCharCode.apply(String, S.slice(0, y))), h.join("")) : String.fromCharCode.apply(String, S.slice(0, y));
    };
    var n = "invalid encoding";
    s.decode = function(o, u, a) {
      for (var h = a, S = 0, y, O = 0; O < o.length; ) {
...
function ri() {
  ...
  function f(n) {
    return typeof Float32Array < "u" ? function() {
      ...
      n.readFloatLE = a ? y : O, n.readFloatBE = a ? O : y;
    }() : function() {
      function o(a, h, S, y) {
        var O = h < 0 ? 1 : 0;
        if (O && (h = -h), h === 0)
      ...
      n.readDoubleLE = a ? y : O, n.readDoubleBE = a ? O : y;
    }() : function() {
      function o(a, h, S, y, O, I) {
        var j = y < 0 ? 1 : 0;
        if (j && (y = -y), y === 0)
          a(0, O, I + h), a(1 / y > 0 ? (
            /* positive */
          ...
          if (y < 22250738585072014e-324)
            L = y / 5e-324, a(L >>> 0, O, I + h), a((j << 31 | L / 4294967296) >>> 0, O, I + S);
          else {
            var w = Math.floor(Math.log(y) / Math.LN2);
            w === 1024 && (w = 1023), L = y * Math.pow(2, -w), a(L * 4503599627370496 >>> 0, O, I + h), a((j << 31 | w + 1023 << 20 | L * 1048576 & 1048575) >>> 0, O, I + S);
          }
        }
      }
      n.writeDoubleLE = o.bind(null, s, 0, 4), n.writeDoubleBE = o.bind(null, r, 4, 0);
      function u(a, h, S, y, O) {
        var I = a(y, O + h), j = a(y, O + S), L = (j >> 31) * 2 + 1, w = j >>> 20 & 2047, k = 4294967296 * (j & 1048575) + I;
        return w === 2047 ? k ? NaN : L * (1 / 0) : w === 0 ? L * 5e-324 * k : L * Math.pow(2, w - 1075) * (k + 4503599627370496);
      }
...
function fe() {
  return br || (br = 1, function(f) {
    var s = f;
    s.asPromise = Qo(), s.base64 = ei(), s.EventEmitter = ti(), s.float = ri(), s.inquire = ni(), s.utf8 = oi(), s.pool = ii(), s.LongBits = ai(), s.isNode = !!(typeof be < "u" && be && be.process && be.process.versions && be.process.versions.node), s.global = s.isNode && be || typeof window < "u" && window || typeof self < "u" && self || it, s.emptyArray = Object.freeze ? Object.freeze([]) : (
...
function Nr() {
  ...
  function n(w, k, R) {
    this.fn = w, this.len = k, this.next = void 0, this.val = R;
  }
  function o() {
  }
  function u(w) {
  ...
  a.create = h(), a.alloc = function(w) {
    return new f.Array(w);
  }, f.Array !== Array && (a.alloc = f.pool(a.alloc, f.Array.prototype.subarray)), a.prototype._push = function(w, k, R) {
    return this.tail = this.tail.next = new n(w, k, R), this.len += k, this;
  };
  function S(w, k, R) {
    k[R] = w & 255;
  }
  ...
  O.prototype = Object.create(n.prototype), O.prototype.fn = y, a.prototype.uint32 = function(w) {
  ...
  function I(w, k, R) {
    for (; w.hi; )
      k[R++] = w.lo & 127 | 128, w.lo = (w.lo >>> 7 | w.hi << 25) >>> 0, w.hi >>>= 7;
    for (; w.lo > 127; )
      k[R++] = w.lo & 127 | 128, w.lo = w.lo >>> 7;
    k[R++] = w.lo;
  }
  a.prototype.uint64 = function(w) {
    ...
    var k = r.from(w).zzEncode();
    return this._push(I, k.length(), k);
  }, a.prototype.bool = function(w) {
    return this._push(S, 1, w ? 1 : 0);
  };
  function j(w, k, R) {
    k[R] = w & 255, k[R + 1] = w >>> 8 & 255, k[R + 2] = w >>> 16 & 255, k[R + 3] = w >>> 24;
  }
  a.prototype.fixed32 = function(w) {
    ...
    return this._push(f.float.writeFloatLE, 4, w);
  }, a.prototype.double = function(w) {
    return this._push(f.float.writeDoubleLE, 8, w);
  };
  var L = f.Array.prototype.set ? function(w, k, R) {
    k.set(w, R);
  } : function(w, k, R) {
  ...
  return a.prototype.bytes = function(w) {
    ...
    if (f.isString(w)) {
      var R = a.alloc(k = t.length(w));
      t.decode(w, R, 0), w = R;
    }
    return this.uint32(k)._push(L, k, w);
  }, a.prototype.string = function(w) {
    var k = e.length(w);
    return k ? this.uint32(k)._push(e.write, k, w) : this._push(S, 1, 0);
    ...
    var w = this.head, k = this.tail, R = this.len;
    return this.reset().uint32(R), R && (this.tail.next = w.next, this.tail = k, this.len += R), this;
  }, a.prototype.finish = function() {
    for (var w = this.head.next, k = this.constructor.alloc(this.len), R = 0; w; )
      w.fn(w.val, k, R), R += w.len, w = w.next;
    return k;
  ...
  var o = typeof Uint8Array < "u" ? function(y) {
    ...
    return f.Buffer ? function(y) {
      return (n.create = function(O) {
        return f.Buffer.isBuffer(O) ? new s(O) : o(O);
      })(y);
    } : o;
  };
...
var mi = (() => {
  ...
  return async function(s = {}) {
    ...
    var w = t.wasmBinary, k, R = !1, Q, te, G, me, we, re, W, wt, Pt, It, jt, St = (c) => c.startsWith("file://");
    function Ct() {
      var c = k.buffer;
      t.HEAP8 = te = new Int8Array(c), t.HEAP16 = me = new Int16Array(c), t.HEAPU8 = G = new Uint8Array(c), t.HEAPU16 = we = new Uint16Array(c), t.HEAP32 = re = new Int32Array(c), t.HEAPU32 = W = new Uint32Array(c), t.HEAPF32 = wt = new Float32Array(c), t.HEAPF64 = jt = new Float64Array(c), t.HEAP64 = Pt = new BigInt64Array(c), t.HEAPU64 = It = new BigUint64Array(c);
    }
    function Ur() {
      if (t.preRun)
    ...
    function Te(c) {
      var p;
      (p = t.onAbort) == null || p.call(t, c), c = "Aborted(" + c + ")", L(c), R = !0, c += ". Build with -sASSERTIONS for more info.";
    ...
    async function Zr(c, l) {
      try {
        var p = await Jr(c), g = await WebAssembly.instantiate(p, l);
        return g;
      } catch (b) {
        L(`failed to asynchronously prepare wasm: ${b}`), Te(b);
      }
    }
    ...
    async function Xr() {
      function c(P, C) {
        return F = P.exports, F = A.instrumentWasmExports(F), k = F.B, Ct(), F.H, Br(), F;
      }
      $r();
      function l(P) {
        return c(P.instance);
    ...
    var Mt = 0, rn = (c, l, p) => {
      ...
      function g(v) {
        var T = p(v);
        T.length !== c.length && Ae("Mismatched type converter count");
    ...
    function an(c, l, p = {}) {
      ...
      if (ue[c] = l, delete De[c], ge.hasOwnProperty(c)) {
        var b = ge[c];
    ...
    function mn(c) {
      ...
      var T = Nt(l, this.registeredClass, v.registeredClass);
      return T === null ? b.call(this) : this.isSmartPointer ? Ee(v.registeredClass.instancePrototype, { ptrType: v, ptr: T, smartPtrType: this, smartPtr: c }) : Ee(v.registeredClass.instancePrototype, { ptrType: v, ptr: T });
    }
    var Ie = (c) => typeof FinalizationRegistry > "u" ? (Ie = (l) => l, c) : (Ke = new FinalizationRegistry((l) => {
      xt(l.$$);
    }), Ie = (l) => {
    ...
    var Le = (c, l) => Object.defineProperty(l, "name", { value: c }), yn = (c, l, p) => {
    ...
    function wn(c, l) {
      ...
      l.$$ || x(`Cannot pass "${ke(l)}" as a ${this.name}`), l.$$.ptr || x(`Cannot pass deleted object as a pointer of type ${this.name}`);
      var p = l.$$.ptrType.registeredClass, g = _e(l.$$.ptr, p, this.registeredClass);
      return g;
    }
    function Pn(c, l) {
      var p;
      ...
      (!l || !l.$$) && x(`Cannot pass "${ke(l)}" as a ${this.name}`), l.$$.ptr || x(`Cannot pass deleted object as a pointer of type ${this.name}`), !this.isConst && l.$$.ptrType.isConst && x(`Cannot convert argument of type ${l.$$.smartPtrType ? l.$$.smartPtrType.name : l.$$.ptrType.name} to parameter type ${this.name}`);
      var g = l.$$.ptrType.registeredClass;
      if (p = _e(l.$$.ptr, g, this.registeredClass), this.isSmartPointer)
    ...
    function In(c, l) {
      if (l === null)
        return this.isReference && x(`null is not a valid ${this.name}`), 0;
      l.$$ || x(`Cannot pass "${ke(l)}" as a ${this.name}`), l.$$.ptr || x(`Cannot pass deleted object as a pointer of type ${this.name}`), l.$$.ptrType.isConst && x(`Cannot convert argument of type ${l.$$.ptrType.name} to parameter type ${this.name}`);
      var p = l.$$.ptrType.registeredClass, g = _e(l.$$.ptr, p, this.registeredClass);
      return g;
    }
    function Re(c) {
    ...
    var jn = () => {
      ...
      }, argPackAdvance: J, readValueFromPointer: Re, fromWireType: mn });
    };
    function Fe(c, l, p, g, b, P, C, v, T, _, M) {
      this.name = c, this.registeredClass = l, this.isReference = p, this.isConst = g, this.isSmartPointer = b, this.pointeeType = P, this.sharingPolicy = C, this.rawGetPointee = v, this.rawConstructor = T, this.rawShare = _, this.rawDestructor = M, !b && l.baseClass === void 0 ? g ? (this.toWireType = wn, this.destructorFunction = null) : (this.toWireType = In, this.destructorFunction = null) : this.toWireType = Pn;
    }
    var zt = (c, l, p) => {
      t.hasOwnProperty(c) || Ae("Replacing nonexistent public symbol"), t[c].overloadTable !== void 0 && p !== void 0 ? t[c].overloadTable[p] = l : (t[c] = l, t[c].argCount = p);
    }, Sn = (c, l, p) => {
      c = c.replace(/p/g, "i");
      var g = t["dynCall_" + c];
      return g(l, ...p);
...
` + b.replace(/^Error(:[^\n]*)?\n/, ""));
      ...
      throw l.forEach(b), new Ut(`${c}: ` + p.map(Vt).join([", "]));
    }, Dn = (c, l, p, g, b, P, C, v, T, _, M, N, U) => {
      M = z(M), P = ne(b, P), v && (v = ne(C, v)), _ && (_ = ne(T, _)), U = ne(N, U);
      var Z = vn(M);
      qe(Z, function() {
        ...
        K = K[0];
        var ie, B;
        g ? (ie = K.registeredClass, B = ie.instancePrototype) : B = Me.prototype;
        var q = Le(M, function(...ot) {
          ...
          var tr = V.constructor_body[ot.length];
          if (tr === void 0)
            throw new ye(`Tried to invoke ctor of ${M} with invalid number of parameters (${ot.length}) - expected (${Object.keys(V.constructor_body).toString()}) parameters instead!`);
          return tr.apply(this, ot);
        }), ae = Object.create(B, { constructor: { value: q } });
        q.prototype = ae;
        var V = new On(M, q, ae, U, ie, P, v, _);
        V.baseClass && ((er = V.baseClass).__derivedClasses ?? (er.__derivedClasses = []), V.baseClass.__derivedClasses.push(V));
        var se = new Fe(M, V, !0, !1, !1), Ne = new Fe(M + "*", V, !1, !1, !1), Qt = new Fe(M + " const*", V, !1, !0, !1);
    ...
    var xe = (c) => {
      ...
      if (c instanceof Tt || c == "unwind")
        return Q;
      S(1, c);
    }, Bt = 0, Ht = () => Et || Bt > 0, Yt = (c) => {
      var l;
      Q = c, Ht() || ((l = t.onExit) == null || l.call(t, c), R = !0), S(c, new Tt(c));
    }, En = (c, l) => {
      Q = c, Yt(c);
    }, Mn = En, Ln = () => {
      if (!Ht())
      ...
      if (!R) {
        if (A.state === A.State.Normal) {
          var l = !1, p = !1;
          c((g = 0) => {
            if (!R && (A.handleSleepReturnValue = g, l = !0, !!p)) {
          ...
          }), p = !0, l || (A.state = A.State.Unwinding, A.currData = A.allocateData(), typeof MainLoop < "u" && MainLoop.func && MainLoop.pause(), xe(() => ko(A.currData)));
        } else A.state === A.State.Rewinding ? (A.state = A.State.Normal, xe(Eo), ee(A.currData), A.currData = null, A.sleepCallbacks.forEach(Jt)) : Te(`invalid state: ${A.state}`);
    ...
    function Zt(c, l, p, g, b, P) {
      var C = l.length;
      C < 2 && x("argTypes array size mismatch! Must at least get return value and 'this' types!"), l[1];
      var v = An(l), T = l[0].name !== "void", _ = C - 2, M = new Array(_), N = [], U = [], Z = function(...K) {
        U.length = 0;
        var ie;
        N.length = 1, N[0] = b;
        for (var B = 0; B < _; ++B)
          M[B] = l[B + 2].toWireType(U, K[B]), N.push(M[B]);
        var q = g(...N);
        function ae(V) {
          if (v)
            Xe(U);
    ...
    var _n = (c, l, p, g, b, P) => {
      ...
      b = ne(g, b), he([], [c], (v) => {
        ...
        }, he([], C, (_) => (_.splice(1, 0, null), v.registeredClass.constructor_body[l - 1] = Zt(T, _, null, b, P), [])), [];
      });
    }, Kt = (c, l, p) => (c instanceof Object || x(`${p} with invalid "this": ${c}`), c instanceof l.registeredClass.constructor || x(`${p} incompatible with "this" of type ${c.constructor.name}`), c.$$.ptr || x(`cannot call emscripten binding method ${p} on deleted object`), _e(c.$$.ptr, c.$$.ptrType.registeredClass, l.registeredClass)), Rn = (c, l, p, g, b, P, C, v, T, _) => {
      l = z(l), b = ne(g, b), he([], [c], (M) => {
        M = M[0];
        var N = `${M.name}.${l}`, U = { get() {
          je(`Cannot access ${N} due to unbound types`, [p, C]);
        }, enumerable: !0, configurable: !0 };
        return T ? U.set = () => je(`Cannot access ${N} due to unbound types`, [p, C]) : U.set = (Z) => x(N + " is a read-only property"), Object.defineProperty(M.registeredClass.instancePrototype, l, U), he([], T ? [p, C] : [p], (Z) => {
          var K = Z[0], ie = { get() {
          ...
          if (T) {
            ...
            var B = Z[1];
            ie.set = function(q) {
              var ae = Kt(this, M, N + " setter"), V = [];
              T(_, ae, B.toWireType(V, q)), Xe(V);
            };
          }
      ...
      var l = Y.toValue(c);
      return et(c), l;
    }, toWireType: (c, l) => Y.toHandle(l), argPackAdvance: J, readValueFromPointer: Re, destructorFunction: null }, Wn = (c) => H(c, Nn), zn = (c, l, p) => {
      switch (l) {
        case 1:
          return p ? function(g) {
      ...
      }, toWireType: (P, C) => C.value, argPackAdvance: J, readValueFromPointer: zn(l, p, g), destructorFunction: null }), qe(l, b);
    }, tt = (c, l) => {
      var p = ue[c];
      ...
      var g = tt(c, "enum");
      l = z(l);
      var b = g.constructor, P = Object.create(g.constructor.prototype, { value: { value: p }, constructor: { value: Le(`${g.name}_${l}`, function() {
      }) } });
      b.values[p] = P, b[l] = P;
    }, Gn = (c, l) => {
      switch (l) {
        case 4:
          return function(p) {
            return this.fromWireType(wt[p >> 2]);
          };
        case 8:
          return function(p) {
            return this.fromWireType(jt[p >> 3]);
          };
        default:
          throw new TypeError(`invalid float width (${l}): ${c}`);
      }
    }, $n = (c, l, p) => {
      l = z(l), H(c, { name: l, fromWireType: (g) => g, toWireType: (g, b) => b, argPackAdvance: J, readValueFromPointer: Gn(l, p), destructorFunction: null });
    }, Bn = (c) => {
      c = c.trim();
      const l = c.indexOf("(");
      ...
      var T = Gt(l, p);
      c = z(c), c = Bn(c), b = ne(g, b), qe(c, function() {
      ...
      }, H(c, { name: l, fromWireType: P, toWireType: _, argPackAdvance: J, readValueFromPointer: Rt(l, p, g !== 0), destructorFunction: null });
    }, Jn = (c, l, p) => {
      ...
      l = z(l), H(c, { name: l, fromWireType(p) {
        ...
        P || g instanceof Uint8Array || g instanceof Uint8ClampedArray || g instanceof Int8Array || x("Cannot pass non-string to std::string"), P ? b = qn(g) : b = g.length;
        var C = rt(4 + b + 1), v = C + 4;
        if (W[C >> 2] = b, P)
          Kn(g, v, b + 1);
        else if (P)
          for (var T = 0; T < b; ++T) {
            var _ = g.charCodeAt(T);
            _ > 255 && (ee(C), x("String has UTF-16 code units that do not fit in 8 bits")), G[v + T] = _;
          }
        else
          for (var T = 0; T < b; ++T)
            G[v + T] = g[T];
      ...
      if (p = g << 1, p - c > 32 && Xt) return Xt.decode(G.subarray(c, p));
      ...
      p = z(p);
      var g, b, P, C;
      l === 2 ? (g = to, b = ro, C = no, P = (v) => we[v >> 1]) : l === 4 && (g = oo, b = io, C = ao, P = (v) => W[v >> 2]), H(c, { name: p, fromWireType: (v) => {
        for (var T = W[v >> 2], _, M = v + 4, N = 0; N <= T; ++N) {
          var U = v + 4 + N * l;
          if (N == T || P(U) == 0) {
            var Z = U - M, K = g(M, Z);
            _ === void 0 ? _ = K : (_ += "\0", _ += K), M = U + l;
          }
        }
        return ee(v), _;
      }, toWireType: (v, T) => {
        typeof T != "string" && x(`Cannot pass non-string to C++ string type ${p}`);
        var _ = C(T), M = rt(4 + _ + l);
        return W[M >> 2] = _ / l, b(T, M + 4, _ + l), v !== null && v.push(ee, M), M;
    ...
    }, po = (c, l, p) => (c = Y.toValue(c), l = tt(l, "emval::as"), uo(l, p, c)), fo = (c, l) => (c = Y.toValue(c), l = Y.toValue(l), Y.toHandle(c[l])), mo = {}, go = (c) => {
      var l = mo[c];
      return l === void 0 ? z(c) : l;
    }, yo = (c) => Y.toHandle(go(c)), ho = (c) => {
      var l = Y.toValue(c);
      Xe(l), et(c);
    }, bo = (c, l) => {
      ...
      for (var g = 1; g <= 4; g *= 2) {
        ...
        var P = Math.min(p, Po(Math.max(c, b), 65536)), C = Io(P);
        if (C)
          return !0;
      }
      return !1;
    ...
    }, gn(), jn(), Ut = t.UnboundTypeError = kn(Error, "UnboundTypeError"), xn();
    var So = { i: rn, s: nn, n: sn, w: ln, f: Dn, d: _n, a: Rn, u: Wn, l: Un, g: Vn, m: $n, b: Hn, e: Yn, c: Jn, v: eo, h: so, x: lo, q: co, j: po, y: et, k: fo, o: yo, A: ho, z: bo, r: Oo, t: jo, p: Yt }, F = await Xr();
    F.C;
    var Co = F.D, rt = t._malloc = F.E, ee = t._free = F.F, To = F.G;
    ...
    var ko = F.ca, Do = F.da, Ao = F.ea, Eo = F.fa;
    function nt() {
      if (ce > 0) {
        Pe = nt;
        return;
      ...
      function c() {
        var l;
        t.calledRun = !0, !R && (Vr(), e(t), (l = t.onRuntimeInitialized) == null || l.call(t), Gr());
      }
...
function zr(f, s) {
  return Math.sqrt((f.x - s.x) ** 2 + (f.y - s.y) ** 2);
}
function gi(f, s) {
  return {
    x: (f.x + s.x) / 2,
...
function yi(f, s) {
  if (f.confidence <= 0 || s.confidence <= 0)
    return { x: 0, y: 0 };
  const r = zr(f.center, s.center), t = gi(f.center, s.center);
  return {
    x: t.x,
    y: t.y + r / 4
...
function hi(f, s, r) {
  if (f.confidence <= 0 || s.confidence <= 0)
    return 0;
  const t = zr(f.center, s.center), e = xr(r.width, r.height);
  return Ot(t / e);
}
function ht(f) {
  const { centerX: s, centerY: r, confidence: t, size: e, status: n } = f;
  return {
    center: {
      x: s,
      y: r
...
function bi(f, s) {
  const { faceCenter: r, faceSize: t } = f, e = qo(t, s), n = {
    topLeft: {
      x: r.x - e,
      y: r.y - e
    },
...
let vi = class extends Ko {
  ...
  parseRawData(s, r) {
    const { brightness: t, sharpness: e } = s.params, { bottomRightX: n, bottomRightY: o, leftEye: u, mouth: a, rightEye: h, topLeftX: S, topLeftY: y } = s, O = ht(u), I = ht(h), j = ht(a);
    return {
      confidence: s.confidence / Ce,
      topLeft: {
        x: S,
        y
      },
