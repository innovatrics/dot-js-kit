...
const Ne = /* @__PURE__ */ new WeakMap(), We = "FinalizationRegistry" in globalThis && new FinalizationRegistry((p) => {
  const a = (Ne.get(p) || 0) - 1;
  Ne.set(p, a), a === 0 && Sr(p);
});
function Do(p, a) {
  const r = (Ne.get(a) || 0) + 1;
  Ne.set(a, r), We && We.register(p, a, p);
}
function Ao(p) {
  We && We.unregister(p);
}
function dt(p, a, r = [], t = function() {
}) {
  let e = !1;
  const n = new Proxy(t, {
    get(o, u) {
...
function Jt(p) {
  const a = p.map(Ve);
  return [a.map((r) => r[0]), Eo(a.map((r) => r[1]))];
}
...
class No {
  ...
  writeImageToMemory(a) {
    ...
    const r = new xo(this.samWasmModule, a);
...
const pt = (p) => Number.parseFloat(p.toFixed(3)), kr = (p, a) => Math.min(p, a);
function Wo(p, a) {
  const r = kr(a.width, a.height);
  return pt(p * r);
}
const Vo = (p) => JSON.parse(
  JSON.stringify(p, (a, r) => typeof r == "number" ? pt(r) : r)
);
...
function ue() {
  return lr || (lr = 1, (function(p) {
    var a = p;
    a.asPromise = zo(), a.base64 = Uo(), a.EventEmitter = Go(), a.float = $o(), a.inquire = Bo(), a.utf8 = Ho(), a.pool = Yo(), a.LongBits = Jo(), a.isNode = !!(typeof ye < "u" && ye && ye.process && ye.process.versions && ye.process.versions.node), a.global = a.isNode && ye || typeof window < "u" && window || typeof self < "u" && self || Xe, a.emptyArray = Object.freeze ? Object.freeze([]) : (
...
function Dr() {
  if (dr) return st;
  dr = 1, st = n;
  var p = ue(), a, r = p.LongBits, t = p.utf8;
  function e(g, O) {
...
var ri = (() => {
  ...
  return (async function(a = {}) {
    ...
    var De = (c, l) => Object.defineProperty(l, "name", { value: c }), an = (c, l, f) => {
...
` + b.replace(/^Error(:[^\n]*)?\n/, ""));
    ...
    var Le = (c) => {
      ...
      A.currData && A.state === A.State.Unwinding && A.exportCallStack.length === 0 && (A.state = A.State.Normal, Le(vo), typeof Fibers < "u" && Fibers.trampoline());
    }, whenDone() {
    ...
    var In = (c, l, f, y, b, P) => {
      ...
      var l = Y.toValue(c);
      return Ye(c), l;
    }, toWireType: (c, l) => Y.toHandle(l), argPackAdvance: J, readValueFromPointer: Ee, destructorFunction: null }, Dn = (c) => H(c, Tn), An = (c, l, f) => {
      switch (l) {
        case 1:
          return f ? function(y) {
      ...
      var y = Je(c, "enum");
      l = V(l);
      var b = y.constructor, P = Object.create(y.constructor.prototype, { value: { value: f }, constructor: { value: De(`${y.name}_${l}`, function() {
      }) } });
      b.values[f] = P, b[l] = P;
    }, Ln = (c, l) => {
      switch (l) {
...
function oi(p, a) {
  if (p.confidence <= 0 || a.confidence <= 0)
    return { x: 0, y: 0 };
  const r = Ar(p.center, a.center), t = ni(p.center, a.center);
  return {
    x: t.x,
    y: t.y + r / 4
...
function ii(p, a, r) {
  if (p.confidence <= 0 || a.confidence <= 0)
    return 0;
  const t = Ar(p.center, a.center), e = kr(r.width, r.height);
  return pt(t / e);
}
function ut(p) {
...
function ai(p, a) {
  const { faceCenter: r, faceSize: t } = p, e = Wo(t, a), n = {
    topLeft: {
      x: r.x - e,
      y: r.y - e
    },
